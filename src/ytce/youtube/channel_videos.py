...
class YoutubeChannelVideosScraper:
    ...
    def get_all_videos(self, channel_id, max_videos=None, show_progress=True):
        ...
        if show_progress:
            print(f"\nOK: Total videos fetched: {len(all_videos)}")
        elif self.debug:
...
if __name__ == "__main__":
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        json.dump({"channel_id": args.channel_id, "total_videos": len(all_videos), "videos": all_videos}, f, indent=2, ensure_ascii=False)

    print(f"\nOK: Videos exported to: {output_file}")
